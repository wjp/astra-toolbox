...
private:
	float *ptr;
	void freeGPUMemory() {
		if (!astraCUDA3d::freeGPUMemory(hnd)) {
			// TODO: Can't easily report failure here
			ASTRA_ERROR("Failed to free memory");
		}
	}
};
...
reducePart(std::unique_ptr<CCompositeGeometryManager::CPart> && base,
	...
	for (TJobSetInternal::iterator i = jobs.begin(); i != jobs.end(); ++i)
	{
		...
		for (SJobInternal &job : L)
		{
			     ...
			     i_out != splitOutput.end(); ++i_out)
			{
				...
				std::unique_ptr<CPart> input = reducePart(std::move(job.pInput), outputPart.get());
           ...
           const CCompositeGeometryManager::CPart *other )
{
	...
	if (base.get()->eType == CCompositeGeometryManager::CPart::PART_PROJ)
		return reduceProjectionPart(dynamic_cast<CCompositeGeometryManager::CProjectionPart*>(base.get()), other);
	else if (base.get()->eType == CCompositeGeometryManager::CPart::PART_VOL)
		return reduceVolumePart(dynamic_cast<CCompositeGeometryManager::CVolumePart*>(base.get()), other);
	else
		assert(false);
}
